...
class MyApplication: UIApplication {
	...
	func rect(forTouchPosition pos: CGPoint) -> CGRect {
		if _touchImage != nil {
			...
			return CGRect(x: pos.x - TOUCH_RADIUS, y: pos.y - TOUCH_RADIUS, width: 2 * TOUCH_RADIUS, height: 2 * TOUCH_RADIUS)
		}
	}
		...
		for touch in event.allTouches ?? [] {
			...
			if touch.phase == .began {
				...
				if delta < MIN_DISPLAY_INTERVAL {
					...
					DispatchQueue.main.asyncAfter(deadline: DispatchTime.now() + Double(Int64(delta * Double(NSEC_PER_SEC))) / Double(NSEC_PER_SEC)) {
