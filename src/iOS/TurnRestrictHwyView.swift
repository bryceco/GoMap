...
class TurnRestrictHwyView: UIView {
    ...
    override func point(inside point: CGPoint, with event: UIEvent?) -> Bool {
        let dist = DistanceFromPointToLineSegment(OSMPointFromCGPoint(point), OSMPointFromCGPoint(centerPoint), OSMPointFromCGPoint(endPoint))
    ...
    func createTurnRestrictionButton() {
        ...
        let location = CGPoint(x: Double(centerPoint.x + (endPoint.x - centerPoint.x) * dist), y: Double(centerPoint.y + (endPoint.y - centerPoint.y) * dist))
    ...
    func createOneWayArrow(atPosition location: CGPoint, isDirection isForward: Bool) {
        ...
        let angle = CGFloat(isForward ? TurnRestrictHwyView.heading(from: location, to: center) : TurnRestrictHwyView.heading(from: center, to: location))
    ...
    class func bearingDegrees(from startingPoint: CGPoint, to endingPoint: CGPoint) -> CGFloat {
        let bearingRadians = atan2(endingPoint.y - startingPoint.y, endingPoint.x - startingPoint.x) // bearing in radians
