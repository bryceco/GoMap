...
class DisplayViewController: UITableViewController {
	...
	@IBAction func chooseAddButtonPosition(_ sender: Any) {
		...
		alert.addAction(UIAlertAction(title: NSLocalizedString("Left side", comment: "Left-hand side of screen"), style: .default, handler: { _ in
			AppDelegate.shared.mapView.mainViewController.buttonLayout = BUTTON_LAYOUT._ADD_ON_LEFT
			self.setButtonLayoutTitle()
		}))
		alert.addAction(UIAlertAction(title: NSLocalizedString("Right side", comment: "Right-hand side of screen"), style: .default, handler: { _ in
