...
private func TouchTranslation(_ touch: UITouch?, _ view: UIView?) -> CGPoint {
    ...
    let delta = CGPoint(x: Double((newPoint?.x ?? 0.0) - (prevPoint?.x ?? 0.0)), y: Double((newPoint?.y ?? 0.0) - (prevPoint?.y ?? 0.0)))
    return delta
}
...
class TapAndDragGesture: UIGestureRecognizer {
    ...
    override func touchesBegan(_ touches: Set<UITouch>, with event: UIEvent) {
        ...
        if tapState == EnumA.need_SECOND_TAP.rawValue && state == .possible {
            ...
            if ProcessInfo.processInfo.systemUptime - lastTouchTimestamp < DoubleTapTime && abs(Float(lastTouchLocation.x - (loc?.x ?? 0.0))) < 100 && abs(Float(lastTouchLocation.y - (loc?.y ?? 0.0))) < 100 {
