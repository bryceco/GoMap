...
// This messes up the button styling on macOS
    ...
    func setGpsState(_ state: GPS_STATE) {
        if mapView.gpsState != state {
            ...
            // update GPS icon
            let imageName = (mapView.gpsState == GPS_STATE.NONE) ? "point.bottomleft.forward.to.point.topright.scurvepath" : "point.bottomleft.forward.to.point.topright.scurvepath.fill"
