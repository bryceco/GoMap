...
final class OsmNoteComment {
	...
	init(fixmeObject object: OsmBaseObject, fixmeKey fixme: String) {
		...
		if let tags = object.tags[fixme] {
			text = "\(friendlyDescription) (\((object.isNode() != nil) ? "node" : (object.isWay() != nil) ? "way" : (object.isRelation() != nil) ? "relation" : "") \(object.ident): \(tags)"
        }
    }
...
final class OsmNote {
	...
	private(set) var noteId: Int64 = 0 // for Notes this is the note ID, for fixme or Keep Right it is the OSM object ID, for GPX it is the waypoint ID
    ...
    init?(gpxWaypointXml waypointElement: DDXMLElement, status: String, namespace ns: String, mapData: OsmMapData) {
        ...
        var description: String = ""
        var osmIdent: OsmIdentifier? = nil
        var osmType: String? = nil
...
final class OsmNotesDatabase: NSObject {
	...
	func updateNotes(forRegion box: OSMRect, fixmeData mapData: OsmMapData, completion: @escaping () -> Void) {
		let url = OSM_API_URL + "api/0.6/notes?closed=0&bbox=\(box.origin.x),\(box.origin.y),\(box.origin.x + box.size.width),\(box.origin.y + box.size.height)"
    ...
    func update(withGpxWaypoints xmlText: String, mapData: OsmMapData, completion: @escaping () -> Void) {
        ...
        DispatchQueue.main.async(execute: { [self] in
			...
			if let namespace1 = DDXMLElement.namespace(withName: "ns1", stringValue: "http://www.topografix.com/GPX/1/0") as? DDXMLElement {
				xmlDoc.rootElement()?.addNamespace(namespace1)
			}
			if let namespace2 = DDXMLElement.namespace(withName: "ns2", stringValue: "http://www.topografix.com/GPX/1/1") as? DDXMLElement {
    ...
    func updateKeepRight(forRegion box: OSMRect, mapData: OsmMapData, completion: @escaping () -> Void) {
        let template = "https://keepright.at/export.php?format=gpx&ch=0,30,40,70,90,100,110,120,130,150,160,180,191,192,193,194,195,196,197,198,201,202,203,204,205,206,207,208,210,220,231,232,270,281,282,283,284,285,291,292,293,294,295,296,297,298,311,312,313,320,350,370,380,401,402,411,412,413&left=%f&bottom=%f&right=%f&top=%f"
		let url = String(format: template, box.origin.x, box.origin.y, box.origin.x + box.size.width, box.origin.y + box.size.height)
    ...
    func updateRegion(_ bbox: OSMRect, withDelay delay: CGFloat, fixmeData mapData: OsmMapData, completion: @escaping () -> Void) {
        workQueue.cancelAllOperations()
    ...
    func enumerateNotes(_ callback: @escaping (_ note: OsmNote) -> Void) {
        (noteForTag as NSDictionary?)?.enumerateKeysAndObjects({ key, note, stop in
    ...
    func update(_ note: OsmNote, close: Bool, comment: String, completion: @escaping (_ newNote: OsmNote?, _ errorMessage: String?) -> Void) {
    ...
    func ignoreList() -> [Int : Bool] {
		if keepRightIgnoreList == nil {
            let path = URL(fileURLWithPath: FileManager.default.urls(for: .libraryDirectory, in: .userDomainMask).map(\.path).last ?? "").appendingPathComponent("keepRightIgnoreList").path
