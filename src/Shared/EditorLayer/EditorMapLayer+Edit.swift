...
extension EditorMapLayer {
	...
	func pasteTags() {
		...
		if selectedPrimary.tags.count > 0 {
			...
			alertPaste.addAction(UIAlertAction(title: NSLocalizedString("Merge Tags", comment: ""), style: .default, handler: { [self] _ in
				self.pasteTagsMerge(selectedPrimary)
			}))
			alertPaste.addAction(UIAlertAction(title: NSLocalizedString("Replace Tags", comment: ""), style: .default, handler: { [self] _ in
	...
	func selectObjectAtPoint(_ point: CGPoint) {
		   ...
		   let hit = osmHitTestNode(inSelectedWay: point, radius: DefaultHitTestRadius)
		{
			...
			// hit test anything
			var segment = -1
			if let hit = osmHitTest(point, radius: DefaultHitTestRadius, isDragConnect: false, ignoreList: [], segment: &segment) {
	...
	func dragFinish(object: OsmBaseObject, isRotate: Bool) {
		...
		if dragState.confirmDrag {
			...
			alertMove.addAction(UIAlertAction(title: NSLocalizedString("Undo", comment: ""), style: .cancel, handler: { _ in
				// cancel move
				self.mapData.undo()
			...
			alertMove.addAction(UIAlertAction(title: NSLocalizedString("Move", comment: ""), style: .default, handler: { _ in
				// okay
			}))
	...
	func deleteCurrentSelection() {
		...
		if selectedRelation?.isMultipolygon() ?? false, selectedPrimary.isWay() != nil {
			...
			alertDelete.addAction(UIAlertAction(title: NSLocalizedString("Cancel", comment: ""), style: .cancel, handler: { _ in
			}))
			...
			alertDelete.addAction(UIAlertAction(title: NSLocalizedString("Detach from relation", comment: ""), style: .default, handler: { [self] _ in
				var error: String?
	...
	func performEdit(_ action: EDIT_ACTION) {
		...
		case .RECTANGULARIZE, .STRAIGHTEN, .REVERSE, .DUPLICATE, .ROTATE, .CIRCULARIZE, .COPYTAGS, .PASTETAGS, .EDITTAGS, .CREATE_RELATION:
			if selectedWay != nil, selectedNode != nil, (selectedNode?.tags.count ?? 0) == 0, (selectedWay?.tags.count ?? 0) == 0, !(selectedWay?.isMultipolygonMember() ?? false) {
		...
		case .RECTANGULARIZE:
			...
			if selectedWay.ident >= 0, !owner.screenLatLonRect().containsRect(selectedWay.boundingBox) {
				...
				let rect: EditAction? = mapData.canOrthogonalizeWay(self.selectedWay!, error: &error)
				if let rect = rect {
					rect()
				}
			}
		case .REVERSE:
			let reverse: EditAction? = mapData.canReverse(selectedWay!, error: &error)
			if let reverse = reverse {
				reverse()
			}
		case .JOIN:
			let join: EditAction? = mapData.canJoin(selectedWay!, at: selectedNode!, error: &error)
			if let join = join {
				join()
			}
		case .DISCONNECT:
			let disconnect: EditActionReturnNode? = mapData.canDisconnectWay(selectedWay!, at: selectedNode!, error: &error)
			if let disconnect = disconnect {
				selectedNode = disconnect()
				owner.placePushpinForSelection(at: nil)
			}
		case .SPLIT:
			let split: EditActionReturnWay? = mapData.canSplitWay(selectedWay!, at: selectedNode!, error: &error)
			if let split = split {
				_ = split()
			}
		case .STRAIGHTEN:
			if let selectedWay = self.selectedWay {
				...
				if selectedWay.ident >= 0, !owner.screenLatLonRect().containsRect(boundingBox) {
					...
					let straighten: EditAction? = mapData.canStraightenWay(selectedWay, error: &error)
		...
		case .CIRCULARIZE:
			let circle: EditAction? = mapData.canCircularizeWay(selectedWay!, error: &error)
		...
		case .CREATE_RELATION:
			...
			actionSheet.addAction(UIAlertAction(title: NSLocalizedString("Multipolygon", comment: ""), style: .default, handler: { _ in
				create("multipolygon")
			}))
	...
	func longPressAtPoint(_ point: CGPoint) {
		...
		if selectedPrimary?.isRelation()?.isMultipolygon() ?? false {
			...
			if ways.count == 1 {
				...
				confirm.addAction(UIAlertAction(title: NSLocalizedString("Add outer member", comment: "Add to relation"), style: .default, handler: { _ in
					addMmember("outer")
				}))
				confirm.addAction(UIAlertAction(title: NSLocalizedString("Add inner member", comment: "Add to relation"), style: .default, handler: { _ in
					addMmember("inner")
				}))
	...
	func extendSelectedWay(to newPoint: CGPoint, from pinPoint: CGPoint) -> Result<CGPoint, EditError> {
		   ...
		   self.selectedNode == nil
		{
			...
			var error: String?
			guard let add = canAddNode(toWay: way, atIndex: segment + 1, error: &error)
			else {
		...
		let prevPrevPoint = prevPrevNode != nil ? owner.mapTransform.screenPoint(forLatLon: prevPrevNode!.latLon, birdsEye: true) : CGPoint.zero
		...
		if way.nodes.count >= 2 {
			...
			if d < 3.0 {
				// join first to last
				var error: String?
				if let action = canAddNode(toWay: way, atIndex: nextIndex, error: &error) {
					action(start)
					selectedWay = way
					selectedNode = nil
					return .success(s)
		...
		var error: String?
		guard let addNodeToWay: EditActionWithNode = canAddNode(toWay: way, atIndex: nextIndex, error: &error)
		else {
			return .failure(.text(error!))
		}
