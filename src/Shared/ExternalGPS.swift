	...
	func processNMEA(_ data: inout Data) {
		while data.count > 8 {
			...
			if line.hasPrefix("PGLL") {
				...
				var dLat = (Double((lat as NSString?)?.substring(to: dot - 2) ?? "") ?? 0.0) + (Double((lat as NSString?)?.substring(from: dot - 2) ?? "") ?? 0.0) / 60.0
				if NS == "S" {
					dLat = -dLat
				}

				dot = (lon as NSString?)?.range(of: ".").location ?? 0
				var dLon = (Double((lon as NSString?)?.substring(to: dot - 2) ?? "") ?? 0.0) + (Double((lon as NSString?)?.substring(from: dot - 2) ?? "") ?? 0.0) / 60.0
				if EW == "W" {
					dLon = -dLon
				}
