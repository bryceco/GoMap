	...
	UIGestureRecognizerDelegate, UISheetPresentationControllerDelegate
{
	...
	func setUpChildViews(with main: MainViewController) {
		...
		AppState.shared.settings.$enableTurnRestriction.subscribe(self) { [weak self] enableTurnRestriction in
			self?.editorLayer.clearCachedProperties()
		}

		AppState.shared.settings.$displayGpxTracks.subscribe(self) { [weak self] displayGpxTracks in
	...
	func discardStaleData() {
		if AppState.shared.settings.enableAutomaticCacheManagement {
	...
	func updateMapMarkersFromServer(withDelay delay: CGFloat, including: MapMarkerDatabase.MapMarkerSet) {
		...
		if including.isEmpty {
			...
			if AppState.shared.settings.displayGpxTracks {
...
extension MapView: EditorMapLayerOwner {
	...
	func useTurnRestrictions() -> Bool {
		return AppState.shared.settings.enableTurnRestriction
	}
	...
	func useAutomaticCacheManagement() -> Bool {
		return AppState.shared.settings.enableAutomaticCacheManagement
	}
