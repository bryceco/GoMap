...
extension OsmMapData {
    ...
    func canAdd(_ obj: OsmBaseObject, to relation: OsmRelation, withRole role: String?, error: inout String?) -> EditAction? {
		var role = role
                        ...
                        if let via = via {
                            if let prevNode = prevNode {
                                if (via.isWay() != nil) && (via == way || (via.isWay()?.nodes.contains(prevNode) ?? false)) {
                                    error = NSLocalizedString("Extending a 'via' in a Turn Restriction will break the relation", comment: "")
                                    return nil
                                }
                            }
                        } else {
                            error = NSLocalizedString("The way belongs to a relation that is not fully downloaded", comment: "")
                            return nil
                        }
                    }
                } else {
                    error = NSLocalizedString("Extending a way which belongs to a Route or similar relation may damage the relation", comment: "")
                    return nil
                }
            }
        }
    ...
    func canOrthogonalizeWay(_ way: OsmWay, error: inout String?) -> EditAction? {
        ...
        return { [self] in
            ...
            var points = [OSMPoint](repeating: OSMPoint(), count: count)
            for i in 0..<count {
            ...
            if count == 3 {
                ...
                var originalPoints = [OSMPoint](repeating: OSMPoint(), count: count)
                memcpy(&originalPoints, points, MemoryLayout.size(ofValue: points))
                var bestScore = 1e9
                ...
                for step in 0 ..< 1000 {
                    ...
                    if newScore < bestScore {
                        memcpy(&bestPoints, points, MemoryLayout.size(ofValue: points))
                        bestScore = newScore
                    }
    ...
    func canMerge(_ node1: OsmNode, into node2: OsmNode, error: inout String?) -> EditActionReturnNode? {
        guard let mergedTags = OsmBaseObject.MergeTagsWith(
            ourTags: node1.tags,
        ...
        for relation in restrictions {
            ...
            let filter = { node in
				return !keyfrom.contains(node) && !keyto.contains(node)
			}
                            ...
                            if viaMember.obj == node {
								   ...
								   let common = viaObject.isWay()?.connectsTo(way: way)
								{
									if common.isNode() == node {
										...
										error = NSLocalizedString("Can't remove Turn Restriction node connecting 'to'/'from' to 'via'", comment: "")
										return false
                                    }
                                }
                            }
    ...
    func canSplitWay(_ selectedWay: OsmWay, at node: OsmNode, error: inout String?) -> EditActionReturnWay? {
        return { [self] in
            ...
            let wayIsOuter = (wayA.isSimpleMultipolygonOuterMember() ? wayA.parentRelations.last : nil) // only 1 parent relation if it is simple
            ...
            for relation in relations {
                if relation.isRestriction() {
                    ...
                    if relation == wayIsOuter {
                        if let merged = OsmBaseObject.MergeTagsWith(ourTags: relation.tags, otherTags: wayA.tags, allowConflicts: true) {
    ...
    func canJoin(_ selectedWay: OsmWay, at selectedNode: OsmNode, error: inout String?) -> EditAction? {
        ...
        for relation in relations {
            ...
            if relation.isRestriction() {
                ...
                if foundSet != 3 {
                    error = NSLocalizedString("Joining would invalidate a Turn Restriction the way belongs to", comment: "")
                    return nil
                }
            }
        ...
        var loc: Int? = nil
        loc = selectedWay.nodes.firstIndex(of: selectedNode) ?? NSNotFound
        if canAddNode(to: selectedWay, at: loc ?? (loc ?? 0) + 1, error: &error) == nil {
            return nil
        }
        loc = otherWay.nodes.firstIndex(of: selectedNode) ?? NSNotFound
        if canAddNode(to: otherWay, at: loc ?? (loc ?? 0) + 1, error: &error) == nil {
            return nil
        }
        
        let newTags = OsmBaseObject.MergeTagsWith(ourTags: selectedWay.tags, otherTags: otherWay.tags, allowConflicts: false)
        if newTags == nil {
            // tag conflict
    ...
    func canCircularizeWay(_ way: OsmWay, error: inout String?) -> EditAction? {
        ...
        return { [self] in
            ...
            radius = AverageDistanceToCenter(way, center)
            
            for i in 0..<(way.nodes.count - 1) {
                let n = way.nodes[i]
                let c = hypot(n.lon - center.x, lat2latp(n.lat) - center.y)
                let lat = latp2lat((center.y ) + (lat2latp(n.lat) - center.y) / c * (radius ?? 0.0))
                let lon = center.x + (n.lon - center.x) / c * (radius ?? 0.0)
            ...
            for var i in 0 ..< way.nodes.count {
                ...
                if clockwise {
                    ...
                    if diff > 20 {
                        var ang = a1 - 20
                        while ang > a2 + 10 {
                            InsertNode(self, way, center, ang, radius ?? 0.0, i + 1)
                            j += 1
                            i += 1
                            ang -= 20
                        }
    ...
    static var isNumericRegex: NSRegularExpression? = nil
    ...
    private func isNumeric(_ s: String) -> Bool {
        ...
        let r = OsmMapData.isNumericRegex?.rangeOfFirstMatch(in: s, options: [], range: NSRange(location: 0, length: s.count))
        return (r?.length ?? 0) > 0
    }
