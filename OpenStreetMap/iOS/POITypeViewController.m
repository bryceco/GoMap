...
- (void)viewDidLoad
{
	...
	PrimaryTagDatabase * types = [PrimaryTagDatabase shared];
	if ( _parentCategory == nil ) {
		_isTopLevel = YES;
		_typeArray = [types primaryTagsForGeometry:geometry];
	} else {
	...
	for ( NSArray * kv in a ) {
		PrimaryTag * tagInfo = [types primaryTagForKey:kv[0] value:kv[1]];
		if ( tagInfo ) {
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
	if ( _searchArrayAll ) {
		...
		PrimaryTag * tagInfo = indexPath.section == 0 ? _searchArrayRecent[ indexPath.row ] : _searchArrayAll[ indexPath.row ];
		NSString * text = tagInfo.friendlyName;
	...
	if ( _isTopLevel && indexPath.section == 0 ) {
		// most recents
		UITableViewCell * cell = [tableView dequeueReusableCellWithIdentifier:@"FinalCell" forIndexPath:indexPath];
		PrimaryTag * tagInfo = _mostRecentArray[ indexPath.row ];
		cell.textLabel.text = tagInfo.friendlyName;
		cell.imageView.image = tagInfo.icon;
		cell.detailTextLabel.text = tagInfo.summary;
		...
		// type array
		PrimaryTag * tagInfo = _typeArray[ indexPath.row ];
		if ( tagInfo.members ) {
...
-(void)updateMostRecentArrayWithSelection:(PrimaryTag *)tagInfo
{
	...
	if ( _isTopLevel && indexPath.section == 0 ) {
		...
		if ( tagInfo.members ) {
			...
			if ( tagInfo.value ) {
				...
				// it is a placeholder for a generic type: line, node, etc.
			}
			[self.navigationController popToRootViewControllerAnimated:YES];
		}
	}
...
- (void)searchBar:(UISearchBar *)searchBar textDidChange:(NSString *)searchText
{
	if ( searchText.length == 0 ) {
		...
		// searching
		PrimaryTagDatabase * types = [PrimaryTagDatabase shared];
		_searchArrayAll = [types primaryTagsForCategory:_parentCategory matching:searchText];
		_searchArrayAll = [_searchArrayAll sortedArrayUsingComparator:^NSComparisonResult(PrimaryTag * t1, PrimaryTag * t2) {
		...
		_searchArrayRecent = [_mostRecentArray filteredArrayUsingPredicate:[NSPredicate predicateWithBlock:^BOOL(PrimaryTag * tagInfo, NSDictionary *bindings) {
